	...
	{:else}
		<div class="grid gap-4">
			{#each perms as perm (perm.id)}
				<div
					class="bg-bg-card border border-white/5 rounded-2xl p-6 hover:border-white/10 transition-all"
				>
					<div class="flex items-center justify-between">
						<div class="flex-1">
							<div class="flex items-center gap-3 mb-2">
								...
								{#if perm.is_open}
									<span
										class="px-3 py-1 rounded-full bg-success/20 text-success text-xs font-bold uppercase"
									>
										Ouverte
									</span>
								{:else}
									<span
										class="px-3 py-1 rounded-full bg-text-muted/20 text-text-muted text-xs font-bold uppercase"
									>
										Fermée
									</span>
								{/if}
						...
						<div class="flex items-center gap-2">
							<button
								...
								class="px-4 py-2 rounded-lg {perm.is_open
									? 'bg-red-500/10 text-red-500 hover:bg-red-500/20'
									: 'bg-success/10 text-success hover:bg-success/20'} font-bold text-xs uppercase transition-all"
							>
...
{#if showCreateModal}
	...
	>
		<div
			class="bg-bg-card border border-white/10 rounded-2xl p-8 max-w-2xl w-full"
			role="presentation"
		...
		>
			...
			<div class="space-y-4 mb-6">
				<div>
					<label for="perm_name" class="block text-sm font-bold text-text-muted mb-2"
				...
				<div>
					<label for="perm_date" class="block text-sm font-bold text-text-muted mb-2">Date</label>
					<input
						id="perm_date"
						type="date"
				...
				<div>
					<span class="block text-sm font-bold text-text-muted mb-2">Barmans</span>
					<UserSearch
						onSelect={(user) => {
					...
					<div class="mt-3 flex flex-wrap gap-2">
						{#each selectedBarmans as barman}
							<div
								class="px-3 py-1 rounded-lg bg-bg-secondary border border-white/5 text-text-primary flex items-center gap-2"
							>
								<span class="text-sm">{barman.prenom} {barman.nom}</span>
								<button
									onclick={() => (selectedBarmans = selectedBarmans.filter((b) => b.id !== barman.id))}
