...
<div class="space-y-4">
	...
	<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
		{#each items as item}
			<button
				...
				class="p-4 border rounded-lg transition-all relative overflow-hidden
                {disabled || item.disabled
					? 'opacity-50 cursor-not-allowed border-border bg-bg-primary'
					: selectedItem?.id === item.id
						? 'border-brand-red bg-brand-red/10'
						: 'border-border hover:border-brand-red/50 bg-bg-secondary'}"
			>
				<div class="text-center">
					...
					{/if}
					{#if item.reason}
						<div class="text-xs text-brand-red font-bold mt-1 uppercase">{item.reason}</div>
					{/if}
				</div>
			</button>
		{/each}
	...
	{#if selectedItem}
		<div class="flex items-center gap-4 p-4 bg-bg-secondary rounded-lg border border-border">
			<div class="flex-1">
				<div class="font-medium text-text-primary">
					{selectedItem.nom || `${selectedItem.contenu_nom} ${selectedItem.contenant_nom}`}
				</div>
				<div class="text-sm text-text-muted">
					{selectedItem.prix_vente.toFixed(2)}€ × {quantity} =
					<span class="font-bold text-brand-red">{(selectedItem.prix_vente * quantity).toFixed(2)}€</span
					>
				</div>
			</div>
			...
			<div class="flex items-center gap-2">
				<button
					onclick={decrement}
					class="w-8 h-8 rounded-lg bg-bg-primary border border-border flex items-center justify-center hover:bg-bg-tertiary transition-colors"
				>
			...
			<button
				...
				class="px-6 py-2 bg-brand-red text-white rounded-lg font-medium hover:bg-red-600 transition-colors"
			>
				Valider
			</button>
		</div>
