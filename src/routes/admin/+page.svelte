<script lang="ts">
	...
	let activeTab = $state('users'); // 'users', 'perms', 'stats'
	let users = $state<any[]>([]);
	let perms = $state<any[]>([]);
	let monthlyStats = $state<any[]>([]);
	...
	async function loadStats() {
		try {
			...
			if (res.ok) monthlyStats = await res.json();
		} catch (e) {
			console.error(e);
		}
	}
...
<div class="min-h-screen pt-24 px-4 md:px-8 max-w-7xl mx-auto pb-32 md:pb-20">
			...
			<div class="p-6 border-b border-border flex flex-col md:flex-row gap-4 justify-between">
				<div class="flex flex-col md:flex-row gap-4 w-full">
					...
					<button
						...
						class="px-4 py-2 bg-brand-red/20 text-brand-red border border-brand-red/30 rounded-xl font-bold hover:bg-brand-red hover:text-white transition-all flex items-center justify-center gap-2"
					>
						<Icon name="UserPlus" size={18} /> Nouveau Membre
					</button>
				</div>
			</div>
							...
							<tr class="hover:bg-bg-secondary/50 transition-colors group">
								<td class="p-4">
									<div class="flex items-center gap-3">
										<div
											class="w-8 h-8 rounded-full bg-bg-tertiary flex items-center justify-center font-bold text-text-primary text-xs"
										>
								...
								<td class="p-4">
									<span
										class="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-bg-secondary text-text-secondary border border-border"
									>
										{user.role}
									</span>
								</td>
								<td
									class="p-4 text-right font-mono font-bold {user.solde < 0 ? 'text-error' : 'text-success'}"
								>
	...
	{#if activeTab === 'perms'}
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" in:fade>
			...
			>
				<div
					class="w-16 h-16 rounded-full bg-brand-yellow/10 group-hover:bg-brand-yellow/20 text-brand-yellow flex items-center justify-center transition-colors"
				>
				...
				>
					<div class="flex justify-between items-start mb-6">
						...
						<div class="flex gap-2 items-start">
							{#if perm.is_open}
								<span
									class="px-2 py-1 bg-success/20 text-success border border-success/20 rounded-lg text-[10px] font-bold uppercase tracking-widest animate-pulse"
									>En cours</span
								>
							{/if}
							<button
								onclick={(e) => editPerm(e, perm)}
								class="p-2 -mr-2 -mt-2 hover:bg-bg-tertiary rounded-lg text-text-muted hover:text-text-primary transition-colors flex z-10"
							>
								<Icon name="Edit2" size={16} />
							</button>
						</div>
					</div>
					<h3 class="text-2xl font-black text-text-primary mb-1 uppercase italic tracking-tighter">
						{perm.nom}
					</h3>
					<p class="text-text-muted font-mono text-sm mb-6">
						{new Date(perm.date * 1000).toLocaleDateString()}
					</p>
					<div class="space-y-2 mt-auto">
						<div
							class="bg-bg-secondary rounded-xl p-3 flex justify-between items-center border border-border"
						>
							<div class="text-[10px] text-text-muted font-bold uppercase">Recettes</div>
							<div class="font-mono font-bold text-text-primary text-sm">
								{formatCurrency(perm.total_vente || 0)}
							</div>
						</div>
						<div
							class="bg-bg-secondary rounded-xl p-3 flex justify-between items-center border border-border"
						>
							<div class="text-[10px] text-text-muted font-bold uppercase">Bénéfice</div>
							<div class="font-mono font-bold text-emerald-400 text-sm">
								{formatCurrency(perm.total_profit || 0)}
							</div>
						</div>
					</div>
				</div>
		...
		<div class="bg-bg-card border border-border rounded-3xl overflow-hidden backdrop-blur-md" in:fade>
			<div class="overflow-x-auto">
				<table class="w-full text-left">
					<thead>
						<tr class="bg-bg-secondary text-text-muted text-[10px] uppercase tracking-widest">
							<th class="p-4">Mois</th>
							<th class="p-4 text-right">CA Total</th>
							<th class="p-4 text-right">Bénéfice</th>
						</tr>
					</thead>
					<tbody class="divide-y divide-border text-sm">
						{#each monthlyStats as stat}
							<tr class="hover:bg-bg-secondary/50 transition-colors">
								<td class="p-4 font-bold text-text-primary">{stat.month}</td>
								<td class="p-4 text-right font-mono font-bold text-text-primary"
									>{formatCurrency(stat.total_global)}</td
								>
								<td class="p-4 text-right font-mono font-bold text-emerald-400"
									>{formatCurrency(stat.total_profit)}</td
								>
							</tr>
						{/each}
					</tbody>
...
{#if showPermModal}
	<AdminPermModal bind:show={showPermModal} perm={selectedPerm} onClose={closeModal} />
{/if}
