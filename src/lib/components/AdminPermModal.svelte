<script lang="ts">
	...
	const stats = $derived.by(() => {
		...
		for (const t of transactions) {
			...
			const prixAchat = isBoisson ? t.boisson_prix_achat || 0 : t.consommable_prix_achat || 0;
			const key = `${t.type}_${t.id_item}`;

			if (!statsMap.has(key)) {
				statsMap.set(key, {
					nom: t.item_nom || t.boisson_nom || t.consommable_nom || 'Inconnu',
			...
			entry.total_achat += t.nb * prixAchat;
...
<Modal bind:show title={`Bilan - ${perm?.nom || 'Perm'}`} onCancel={onClose} hideActions={true}>
	<div class="space-y-6">
		<div class="grid grid-cols-3 gap-4">
			<div class="p-4 bg-bg-secondary rounded-xl border border-white/5 text-center">
				<div class="text-xs text-text-muted uppercase font-bold mb-1">Recettes</div>
				<div class="text-2xl font-black text-text-primary">{formatCurrency(totalRecettes)}</div>
			</div>
			<div class="p-4 bg-bg-secondary rounded-xl border border-white/5 text-center">
				<div class="text-xs text-text-muted uppercase font-bold mb-1">Bénéfice</div>
				<div class="text-2xl font-black text-emerald-400">
					{formatCurrency(totalRecettes - stats.reduce((acc, c) => acc + c.total_achat, 0))}
				</div>
			</div>
			<div class="p-4 bg-bg-secondary rounded-xl border border-white/5 text-center">
				<div class="text-xs text-text-muted uppercase font-bold mb-1">Articles</div>
				<div class="text-2xl font-black text-text-primary">
					{stats.reduce((acc, c) => acc + c.count, 0)}
				</div>
			</div>
		</div>
		...
		<div class="overflow-hidden border border-white/5 rounded-xl">
			<table class="w-full text-left text-sm">
				<thead>
					<tr
						class="bg-bg-secondary text-text-muted text-xs uppercase tracking-wide border-b border-white/5"
					>
						<th class="p-3">Qté</th>
						<th class="p-3">Nom</th>
						...
						{#each stats as line}
							<tr class="hover:bg-bg-secondary/50">
								...
								<td class="p-3 text-right font-mono text-xs"
									>{line.total_litre > 0 ? `${line.total_litre.toFixed(2)}L` : '-'}</td
								>
